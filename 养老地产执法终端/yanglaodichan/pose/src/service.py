...
import algorithm
from multi_person_pose import pose_data
...
# 例：http://127.0.0.1:8000/ai/alphapose/skeletonpose/v1.0
...
class ServiceHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            if url.path == '/ai/alphapose/skeletonpose/v1.0':
                print('骨骼坐标')

                data = pose_data(request_data['filePath'], request_data['subjectNumber'],
                                 request_data['inputType'], request_data['framePerSecond'])
                response_data = {'requestID': request_data['requestID'], 'data': data}
                print(request_data)
                response(self, response_data)

                logging(log, url, response_data)
            elif url.path == '/ai/alphapose/onelegedstand/v1.0':
                print('单脚站立时长')
            elif url.path == '/ai/alphapose/swingfrequency/v1.0':
                print('单脚站立时身体摆动频率')
            else:
