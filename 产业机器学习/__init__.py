...
class BarCodeReader(object):
    ...
    def decode(self, filename, try_harder=False, possible_formats=None, charset=''):
        ...
        if not p.returncode:
            return BarCode.parse(stdout, charset)
...
class BarCode(object):
    @classmethod
    def parse(cls, zxing_output, charset):
        ...
        if charset != '':
            raw = raw[:-1].decode('GBK')
            parsed = parsed[:-1].decode('GBK')
        else:
            raw = raw[:-1].decode()
